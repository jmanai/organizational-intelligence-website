...
try {
  ...
  assert.equal(calls.length, 2, 'Invalid and honeypot requests must not contact HubSpot or send email');
  await onRequestPost({ request: request({...valid, anythingElse: '<script>alert(1)</script>\nSecond line'}), env });
  assert.ok(calls[3].body.html.includes('&lt;script&gt;alert(1)&lt;/script&gt;'));
  assert.ok(!calls[3].body.html.includes('<script>'));
  assert.ok(calls[3].body.text.includes('<script>alert(1)</script>\nSecond line'));
